...
BEGIN
    ...
    FOR score_record IN 
        SELECT 
            ...
            fs.total_gross,
            r.course_name,
            r.tee_name || '-' || r.tee_color as tee_name,
            p.id as player_id
    ...
    LOOP
        ...
        SELECT ph.handicap INTO player_handicap
        FROM player_handicaps ph
        WHERE ph.player_id = score_record.player_id
        AND ph.course_name = score_record.course_name
        AND ph.tee_name = score_record.tee_name;

        IF FOUND THEN
            -- Update the finalized score with the correct handicap and net score
            UPDATE finalized_scores
            SET course_handicap = player_handicap,
                total_net = score_record.total_gross - player_handicap
