...
DROP FUNCTION IF EXISTS trigger_calculate_match_results();
...
BEGIN
    ...
    WITH team_a_hole_scores AS (
        SELECT 
            s.hole_number,
            -- For 2v2, take the best score between partners for each hole
            MIN(CASE WHEN v_match.scoring_type = 'net' THEN ns.net_score ELSE s.strokes END) as best_score
        FROM match_players mp
        JOIN scores s ON s.auth_id = mp.auth_id AND s.round_id = v_match.round_id
...
-- Create trigger function to recalculate match results
